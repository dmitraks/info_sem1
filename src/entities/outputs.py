...
class Outputs:
    ...
    def json_ser(self) -> None:
        with open('out/workers.json', 'w') as f: f.write('')
        data = []
        ...
        with open('out/workers.json', 'a', encoding='UTF-8') as f:
            json.dump(data, f, indent='    ', ensure_ascii=False)
        return None


    def csv_ser(self) -> None:
        with open('out/workers.csv', 'w') as f: f.write('')
        data = []
        for worker in self.workers.workers:
        ...
        with open('out/workers.csv', 'a', encoding='UTF-8') as f:
            writer = csv.writer(f)
    ...
    def xml_ser(self) -> None:
        with open('out/workers.xml', 'w') as f: f.write('')
        data = []
        for worker in self.workers.workers:
            rights = {}
        ...
        indent(tree, space='    ')
        tree.write('out/workers.xml', encoding='UTF-8', xml_declaration=True)
        return None
    ...
    def yaml_ser(self) -> None:
        with open('out/workers.yaml', 'w') as f: f.write('')
        with open('out/workers.yaml', 'a', encoding='UTF-8') as f:
